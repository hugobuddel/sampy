...
class SAMPMsgReplierWrapper(object):
  ...
  def __call__(self, f):
    ...
    def wrapped_f(*args):
      if (inspect.ismethod(f) and f.__func__.__code__.co_argcount == 6) or \
         (inspect.isfunction(f) and f.__code__.co_argcount == 5) or \
         args[2] is None:
    ...
    def do_POST(self):
      ...
      try:
        ...
        if method == "samp.webhub.register":
          ...
          params = tuple(params)
          data = xmlrpc.client.dumps(params, methodname=method)
...
<access-policy>
  ...
  def do_GET(self):
    ...
    if split_path[0] in ['/translator/%s' % clid for clid in self.server.clients]:
      ...
      urlpath = urllib.parse.parse_qs(split_path[1])
      try:
        proxyfile = urllib.request.urlopen(urlpath["ref"][0])
        self.send_response(200, 'OK')
        self.end_headers()
...
class SAMPHubServer(object):
  ...
  def _timeoutTestClient(self):
    ...
    while self._is_running:
      ...
      if self._client_timeout > 0:
        now = time.time()
        for private_key in list(self._client_activity_time.keys()):
  ...
  def _createLockFile(self):
    ...
    if "SAMP_HUB" in os.environ:
      ...
      if os.environ["SAMP_HUB"].startswith("std-lockurl:"):
        ...
        lockfile_parsed = urllib.parse.urlparse(lockfilename)
  ...
  def checkRunningHub(lockfilename):
    ...
    if "samp.hub.xmlrpc.url" in lockfiledict:
      try:
        proxy = xmlrpc.client.ServerProxy(lockfiledict["samp.hub.xmlrpc.url"].replace("\\", ""),
                                      allow_none=1)
        proxy.samp.hub.ping()
        is_running = True
  ...
  def _setXmlrpcCallback(self, private_key, xmlrpc_addr):
    ...
    if private_key in self._private_keys:
      ...
      if SSL_SUPPORT and xmlrpc_addr[0:5] == "https":
        ...
        server_proxy = xmlrpc.client.ServerProxy(xmlrpc_addr, allow_none=1)
  ...
  def _unregister(self, private_key):
    ...
    for mtype in list(self._mtype2ids.keys()):
  ...
  def _getRegisteredClients(self, private_key):
    ...
    if private_key in self._private_keys:
      reg_clients = []
      for pkey in list(self._private_keys.keys()):
        if pkey != private_key:
    ...
    if responder_private_key in self._private_keys and msg_id:
      ...
      try:
        ...
        if recipient_msg_tag == "sampy::sync::call":
          if msg_id in list(self._sync_msg_ids_heap.keys()):
  ...
  def _getPrivateKeyFromPublicId(self, public_id):
    ...
    for private_key in list(self._private_keys.keys()):
  ...
  def connect(self, hub_params = None, user = None, password = None,
    ...
    try:
      ...
      if SSL_SUPPORT and url[0:5] == "https":
        ...
        self.proxy = xmlrpc.client.ServerProxy(url, allow_none=1)
  ...
  def _declareSubscriptions(self, subscriptions = None):
    if self.hub.isConnected() and self._private_key != None:
      ...
      try:
        ...
        for mtype in list(self._notification_bindings.keys()):
          mtypes_dict[mtype] = copy.deepcopy(self._notification_bindings[mtype][1])
          
        # Collect notification mtypes and metadata
        for mtype in list(self._call_bindings.keys()):
...
SAMPProxyError = xmlrpc.client.Fault
